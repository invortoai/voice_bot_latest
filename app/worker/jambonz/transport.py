...
class JambonzTransportParams(TransportParams):
    """Configuration parameters for Jambonz websocket transport."""
...
class JambonzTransportClient:
    """Client wrapper for handling FastAPI/Starlette WebSocket connections.

    Unlike Pipecat's built-in FastAPI websocket client wrapper, this one can
    *receive and send both binary and text messages* (Jambonz-style protocols
    commonly mix JSON control messages with binary audio).
    """
...
class JambonzInputTransport(BaseInputTransport):
    """Input transport for Jambonz-style websocket connections."""
...
class JambonzOutputTransport(BaseOutputTransport):
    """Output transport for Jambonz-style websocket connections."""
    ...
    async def start(self, frame: StartFrame):
        ...
        # Simulate audio device timing, like Pipecat's websocket transport.
        audio_bytes_10ms = (
...
class JambonzTransport(BaseTransport):
    """Jambonz-style websocket transport for real-time audio streaming."""
